        ...
        <form method="POST" action="{{ route("admin.orders.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('carriage') ? 'is-invalid' : '' }}" name="carriage_id" id="carriage_id" required>
                    @foreach($carriages as $id => $entry)
                        <option value="{{ $id }}" {{ old('carriage_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('received') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="received" id="received" value="1" {{ old('received', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('preparing') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="preparing" id="preparing" value="1" {{ old('preparing', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('delivered') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="delivered" id="delivered" value="1" {{ old('delivered', 0) == 1 ? 'checked' : '' }}>
